  ...
  // console.log(location);
     if(location.pathname === '/'){
      ...
      setSize({...size,
        ...
        mainScreen:'sm:basis-1/2 lg:basis-1/2 md:basis-2/5 xs:basis-full top w-s'})
        document.body.style.overflowY = "";
     }
     ...
     console.log('Auth',auth)
      ...
      setSize({...size,
        ...
        mainScreen:'sm:basis-1/2 lg:basis-3/5	 md:basis-2/5 xs:basis-full top w-s'})
         document.body.style.overflowY = "";
      }
      else if(location.pathname === '/profile'){
        setSize({...size,
          sidebar:'lg:basis-1/5 sidebar_profile',
          mainScreen:'sm:basis-1/2 lg:basis-basis-3/5	md:basis-2/5 xs:basis-full top w-s'})
        document.body.style.overflowY = "";
     }
},[location.pathname])
...
useEffect(()=>{
UserProfile.userGet(auth)
.then((res)=>{
  let { data }=res
  const { userName,profilePicture,isStorie } =data

  let setProfilePicture=profilePicture.length>0?profilePicture[0]:DummyPic
          ...
          <div className='relative'>
            ...
            { value.length===0? <div className='text-right pr-2 cursor-pointer' onClick={()=>setSearchBox(false)}><p>x</p></div>:<></>}
             ...
             return (
              ...
              <div className=' border-gray-400 py-2 cursor-pointer' onClick={()=>setSearchBox(false)} key={index}>
