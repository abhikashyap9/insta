...
const addPost=()=>{
  if(isShow && !isVideo){
         ...
         <div className='pb-12'>
            ...
            {/* .....logo and add access to camera and microphone.. */}
              <div className=''>
                <div><img src='' alt=''/></div> 
                <div>
                 <h1 className='text-center'>Allow access to your camera and microphone </h1>
              ...
              <div className='flex'>
                <div className='basis-2/5 px-4'>
                  <div className='h-6 w-6 border-2 border-b-white rounded '>
                ...
                <div className='cursor-pointer flex basis-3/5'>
                  ...
                  <p className='px-2' onClick={addReel}>Reel</p>
                </div>
              </div>
            </div>
          </div>
    ...
    return (
      <>
        ...
        <React.Fragment>
          ...
          <video src={videoSrc} ref={videoRef} onTimeUpdate={handlePauseVideo} className='w-2/4 m-auto'>
            <source src={videoSrc} type={videoFileValue.type} />
          </video>
          ...
          <p className='text-white'>{convertToHHMMSS(startTime)}</p> &nbsp; End duration:{' '}
          <p className='text-white'>{convertToHHMMSS(endTime)}</p>
          </div>
          <br />
          <button onClick={handlePlay} className='text-white'>Play</button> &nbsp;
          <button onClick={handleTrim} className='text-white'>Trim</button>
          <br />
        </React.Fragment>
      ) : (
        ''
    ...
    return(
      <>
      <button onClick={shareVideo} className='text-white'>Share</button> 
    ...
    return(
      <>
      <div className='relative bg-zinc-900 h-36 w-24' onClick={()=>setisShow(true)}>
      <p className='text-white absolute bottom-0 p-1 text-sm font-medium'>Camera</p>
     </div>
     </>
    )
  }
...
const shareVideo=()=>{
  ...
  Video.addVideo(formdata,auth).then((res)=>{
  ...
  return (
    <>
      { isOpen && <ReactDialogBox
         ...
         <div className='py-4 text-white cursor-pointer' onClick={() => setIsOpen(false)}><h1>Cancel</h1></div>
          </ReactDialogBox>
        }
    ...
    <div className='lg:basis-3/5 sm:w-full'>
      <Stories addStories={modal}/>
      {
       userPost.length>0 ? userPost?.map((current, index) => {
          return (
            <Posts
