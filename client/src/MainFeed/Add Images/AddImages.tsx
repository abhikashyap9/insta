...
function AddImages(props) {
  ...
  let leftCaption =
    capPage === false && selectedImages.length > 0 ? (
      <IoIosArrowDropleft onClick={() => setSelectedImages([])} />
    ) : capPage === true && selectedImages.length > 0 ? (
      <IoIosArrowDropleft onClick={() => setCapPage(false)} />
    ) : (
      ''
    )
  let rightCaption =
  ...
  const getLocationValues = (e) => {
    ...
    const text =
      getLocationValuess
        ...
        .replace(/[\W_]+/g, '')
        .charAt(0)
        .toUpperCase() + getLocationValuess.slice(1)
  ...
  return (
    <div
      className="border border-gray-200 rounded-lg bg-white lg:w-full xs:w-full sm:w-full mt-5"
      onClick={addImage}
    >
      <Header
