...
const SignupForm = () => {
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="mt-4">
      ...
      <div className={ state===3 ? 'slw' : 'slws' } onClick={()=>setMyState(3)}>
      <input
        id="userName"
        name="userName"
        type="text"
      ...
      <div className={ state===4 ? 'slw' : 'slws' } onClick={()=>setMyState(4)}>
      <input 
             id="password"
             name="password" 
             type="password"
     ...
     <div className='text-center text-white'><button type="submit" className='bg-sky-500 px-4 py-1 rounded-sm w-full'>Signup</button></div>
    </form>
  );
};
