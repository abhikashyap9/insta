	...
	async Signup(req:Request,res:Response){
		  const userData=req.body
			try {
				...
				return res.status(201).json({data:user,message:'User created',status:1}).end()
			} catch (err) {
				console.log(err)
				return res.status(400).send({error:err,message:'Something went wrong',status:0}).end()
			}
...
// 	return res.status(400).send({error: 'user name is short',message:'user name is short',status:0})
// 	}

// 	if(fullName.length<6){
// 		return res.status(400).send({error: 'full name is short',message:'user name is short',status:0})
// 	}
...
// 		if (userEmailExist) {
// 			return res.status(409).send({error: 'User Email Already Exist',message:'User Email Already Exist',status:0})
// 		}
	
// 		if (userNameExist) {
// 		return res.status(409).send({error: 'User Name Already Exist',message:'User Name Already Exist',status:0})
// 		}
...
// 		signupConfirmation(userName,email,otp)

// 		return res.status(201).json({data:savedUser,message:'User created',status:1}).end()
// 	} catch (err) {
// 		console.log(err)
// 		return res.status(400).send({error:err,message:'Something went wrong',status:0}).end()
// 	}
...
// 			const saltRounds = 10
// 			const hashedPassword = await bcrypt.hash(password, saltRounds)
// 			const userData = await Signupuser.findByIdAndUpdate(
// 				{_id: tokenData?._id},
...
// 				'<p>Hi ' +
// 				name +
// 				' ,Please copy the link <a href="http://localhost:3000/resetPassowrd?token=' +
// 				token +
